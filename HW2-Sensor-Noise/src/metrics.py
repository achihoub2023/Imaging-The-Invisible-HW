...
def calc_SNR_for_specific_gain(mean,var):
    ...
    for j in range(255):
        ...
        #check if there are any pixels with the mean value, otherwise just zero it out
        if len(indices[0]) > 0: 
            #square root of the variance
            std_devs = np.sqrt(var[indices])  
            avg_std_dev = np.mean(std_devs) 
            
            #check if the average standard deviation is greater than 0, avoid divide by zero, SET TO -1 to match graph, otherwise this should be zero
            if avg_std_dev > 0:
                SNR[j] = j / avg_std_dev  
            else:
                SNR[j] = -1

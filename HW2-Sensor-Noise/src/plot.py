...
def plot_input_histogram(imgs,sensitivity):
    ...
    #we should iterate here bc we have rows we want to set up, idk why 50 bins gets close enough
    num_rows = (num_histograms + 1) // 2
    plt.figure(figsize=(15, 5 * num_rows))

    for i in range(num_histograms):
        plt.subplot(num_rows, 2, i + 1)
        plt.hist(imgs[:, :, i].ravel(), bins=100, alpha=0.8, ec="k")
...
def plot_histograms_channels(img,sensitivity):
    ...
    ax[0].hist(flattened, bins=50, alpha=0.8, range=(0, 250))
    ax[0].set_title('Sensitivity for Red Channel')
    ax[0].set_ylabel('Count')
    
    flattened_2 = img[:, :, 1].flatten()
    ax[1].hist(flattened_2, bins=50, alpha=0.8, range=(0, 250))
    ax[1].set_title('Sensitivity for Green Channel')
    ax[1].set_ylabel('Count')
    
    flattened_3 = img[:, :, 2].flatten()
    ax[2].hist(flattened_3, bins=50, alpha=0.8, range=(0, 250))
